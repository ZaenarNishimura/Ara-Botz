let handler = async (m, { conn, text }) => {
  ...
  try {
    ...
    if(user === conn.user.jid) return conn.reply(m.chat, `*Tidak bisa berpacaran dengan bot. Soalnya laki*`, m)
    ...
    if(global.db.data.users[user].pasangan != m.sender){
      ...
      conn.reply(m.chat,`*Anda baru saja telah menolak @${user.split('@')[0]} Buset😂 Kasian*`,m,{contextInfo: {
        mentionedJid: [user]
      }})
    }
	}	
